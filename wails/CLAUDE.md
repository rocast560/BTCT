# BTCT desktop app: architecture and invariants

> **Structure note (read first).** This guide was written when BTCT lived at the
> repo root and shipped as a Docker image. It is now the **Wails desktop app**
> under `wails/` (see the repo-root `CLAUDE.md` and `wails/README.md`). Paths in
> this document map as follows:
> 
> - `src/...` (the React client) is now **`frontend/src/...`** (run client
>   commands from `wails/frontend`).
> - `server/...` (the Bun server) is now **`wails/server/...`** and runs as a
>   sidecar spawned by `wails/app.go`, not from Docker.
> - The **Docker / `Dockerfile` / `docker-compose` / `*.ps1`** material below is
>   **historical**: that packaging was removed when the app became a Wails shell.
>   The backup engine, AI assistant, MCP server, Yjs relay, and every data-layer
>   invariant still apply unchanged inside `wails/server`.
