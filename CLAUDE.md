...
13. **A skin must not create stacking contexts on the rails or the content column** (`[data-ui="sidebar"]`, `[data-ui="main"]`: no `backdrop-filter`, `filter`, `transform`, `isolation`, `z-index`). Dialogs render inside them with `position: fixed`; a stacking context traps them behind later siblings. Blur goes on a `::before` with `z-index: -1`, and the rails' mount animation must end at `transform: none` and full opacity for the same reason. And because unlayered rules beat Tailwind's hover utilities, every surface a skin restyles needs its own `:hover`.
