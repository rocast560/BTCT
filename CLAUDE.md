...
- **Framing is a fixed viewport, not a free crop.** The editor frames the image with the figure box's real proportions and pans/scales the image behind it. The stored `CropRect` therefore has two rules the old free-crop model lacked: its aspect ratio always equals the box's, and **it may extend outside 0..1** (an image scaled smaller than the frame). Nothing clamps it to the unit square — `lib/crop-math.ts` is built around that. Gaps are baked in as `luma(245)` by the canvas so the editor and the PDF are byte-identical.
...
3c. **Pane resizing in the Typst tab writes `style.width` directly** (one mutation per rAF) and commits to React state only on pointer-up — same reason as #3: a mid-drag re-render can remount the editor and drop the collab binding. Width math + persistence live in `src/lib/pane-resize.ts` (pure, tested).
