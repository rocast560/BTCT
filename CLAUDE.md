...
13. **A skin must not create stacking contexts on the rails or the content column** (`[data-ui="sidebar"]`, `[data-ui="main"]`: no `backdrop-filter`, `filter`, `transform`, `isolation`, `z-index`). Dialogs render inside them with `position: fixed`; a stacking context traps them behind later siblings. Blur goes on a `::before` with `z-index: -1`, and the rails' mount animation must end at `transform: none` and full opacity for the same reason. **A skin must never reset `transform` on `*`** (not even under `prefers-reduced-motion`): the Typst preview is an SVG positioned by `transform` attributes, which a CSS transform overrides (the page collapses into a black blob), and React Flow positions its viewport and nodes with inline transforms; reset only the transforms the skin itself sets (`src/test/glass-theme.test.ts` guards this). And because unlayered rules beat Tailwind's hover utilities, every surface a skin restyles needs its own `:hover`.
