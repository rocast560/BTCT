...
// ── Web recon / site maps ──
// Ingest a normalized "BTCT sitemap JSON" document (from the external recon
// script's token POST or from an in-app server-side scan) into the shared doc.
// Creates/updates the SiteMap (name is a collaborative Y.Text, invariant #1),
// then upserts nodes by their natural `key` and edges by their endpoints, so a
// re-scan merges instead of duplicating. Nodes/edges are LWW JSON with no
// Y.Text fields (like typstAssets/commandLogs), so invariant #1 doesn't apply
// to them. Mirrors the client repo in src/db/sitemap-repo.ts.
function sitePos(index) {
...
export async function ingestSiteMap({ workspaceId, siteMapId, name, rootUrl, nodes = [], edges = [], scannedAt }, actor) {
  const { doc, tables, texts } = await shared();
  const now = Date.now();
  const stamped = Number.isFinite(Number(scannedAt)) ? Number(scannedAt) : now;

  // Resolve the workspace: explicit → the map's own → first workspace.
  let wsId = workspaceId || '';
  let map = siteMapId ? tables.siteMaps.get(siteMapId) : null;
  if (map && !wsId) wsId = map.workspaceId;
  if (!wsId) {
    for (const w of tables.workspaces.values()) { wsId = w.id; break; }
  }
  if (!wsId) throw new Error('no target workspace');

  const result = { siteMapId: null, nodes: 0, edges: 0 };

  doc.transact(() => {
    // 1. Create-or-update the map.
    let mapId = siteMapId && map ? siteMapId : null;
    if (!mapId) {
      mapId = uuid();
      const nm = name || rootUrl || 'Web Map';
      tables.siteMaps.set(mapId, {
        id: mapId, workspaceId: wsId, name: nm, rootUrl: rootUrl || '',
        lastScanAt: stamped, createdAt: now, updatedAt: now,
      });
      setText(texts, textKey('siteMap', mapId, 'name'), nm);
    } else {
      const cur = tables.siteMaps.get(mapId);
      tables.siteMaps.set(mapId, {
        ...cur,
        rootUrl: cur.rootUrl || rootUrl || '',
        lastScanAt: stamped,
        updatedAt: now,
      });
    }
    result.siteMapId = mapId;

    // 2. Upsert nodes by key.
    const existing = [];
    for (const n of tables.siteMapNodes.values()) if (n && n.siteMapId === mapId) existing.push(n);
    const byKey = new Map(existing.map((n) => [n.key, n]));
    const idByKey = new Map();
    let addIndex = existing.length;
    for (const inp of nodes) {
      if (!inp || !inp.key) continue;
      const ex = byKey.get(inp.key);
      if (ex) {
        idByKey.set(inp.key, ex.id);
        tables.siteMapNodes.set(ex.id, {
          ...ex,
          url: inp.url || ex.url,
          method: inp.method || ex.method,
          status: inp.status != null ? inp.status : ex.status,
          contentType: inp.contentType || ex.contentType,
          title: inp.title || ex.title,
          size: inp.size != null ? inp.size : ex.size,
        ...
        tables.siteMapNodes.set(id, {
          id, siteMapId: mapId, key: inp.key,
          type: inp.type || 'page',
          url: inp.url || '', method: inp.method || '',
          status: inp.status != null ? inp.status : null,
          contentType: inp.contentType || '', title: inp.title || '',
          size: inp.size != null ? inp.size : null,
          params: Array.isArray(inp.params) ? inp.params : [],
          sources: Array.isArray(inp.sources) ? inp.sources : [],
          tags: Array.isArray(inp.tags) ? inp.tags : [],
          notes: inp.notes || '',
          position: sitePos(addIndex++),
    ...
    // 3. Upsert edges (resolve keys → ids, dedupe by source|target|kind).
    const existingEdges = [];
    for (const e of tables.siteMapEdges.values()) if (e && e.siteMapId === mapId) existingEdges.push(e);
    const seenEdge = new Set(existingEdges.map((e) => `${e.sourceNodeId}|${e.targetNodeId}|${e.kind}`));
    for (const inp of edges) {
      if (!inp) continue;
      const src = idByKey.get(inp.source);
      const tgt = idByKey.get(inp.target);
      if (!src || !tgt || src === tgt) continue;
      const kind = inp.kind || 'link';
      const dedupe = `${src}|${tgt}|${kind}`;
      if (seenEdge.has(dedupe)) continue;
      seenEdge.add(dedupe);
      const id = uuid();
      tables.siteMapEdges.set(id, {
        id, siteMapId: mapId, sourceNodeId: src, targetNodeId: tgt,
        kind, label: inp.label || '', createdAt: now, updatedAt: now,
      });
      result.edges++;
    }

    logChange(tables, wsId, 'update', 'page', mapId, `Web recon: +${result.nodes} nodes, +${result.edges} edges`, actor);
  });
